...
import {
  ...
  TOKEN_PROGRAM_ID,
} from "./token-program-instruction.service";
...
export class TokenProgramService {
  ...
  static async createTokenAccount(
    ...
    const lamportsToInitializeAccount =
    ...
    const initializeTokenAccountInstruction =
      TokenProgramInstructionService.initializeAccount(
        userAddress,
        tokenMintAddress,
    ...
    console.info(
      `Created Token Account ${tokenAccount.publicKey.toBase58()}`,
      "---",
      txSign,
      "\n"
    );
  ...
  static async createTokenMint(
    ...
    if (
      await SolanaService.isAddressInUse(connection, tokenMintAccount.publicKey)
    ) {
      console.info(
        `SKIPPED: Token Mint ${tokenMintAccount.publicKey.toBase58()} is already existed`,
        "\n"
      );
    ...
    console.info(
      `Created Token Mint ${tokenMintAccount.publicKey.toBase58()}`,
      "---",
      txSign,
      "\n"
    );
  ...
  static async createNonFungibleTokenMint(
    ...
    const initialOwnerTokenAddress =
    ...
    const disableMintAuthorityInstruction =
      TokenProgramInstructionService.changeAuthority(
        payerAccount.publicKey,
        tokenMintAccount.publicKey,
        0,
        null
  ...
  static async findRecipientTokenAddress(
    ...
    const recepientType = await this.checkAddressType(
      connection,
      recipientAddress
    );
    if (recepientType === 1) {
  ...
  static async migrateSplTokenAccounts(
    ...
    const tokenAccountInfos: TokenAccountInfo[] =
      userTokenAccountsResult.value.map((tokenAccount) => {
        const result = TokenProgramInstructionService.decodeTokenAccountInfo(
          tokenAccount.account.data
        );
        result.address = tokenAccount.pubkey;
        return result;
      });
    const tokenMintAddresses: PublicKey[] = tokenAccountInfos
      .map((account) => account.mint)
      .filter((value, index, self) => {
        return (
          self.findIndex(
            (subValue) => subValue.toBase58() === value.toBase58()
          ) === index
        );
      });
    for (let i = 0; i < tokenMintAddresses.length; i++) {
      const tokenMintAddress = tokenMintAddresses[i];
      const filteredTokenAccountInfos = tokenAccountInfos.filter(
        (accountInfo) =>
          accountInfo.mint.toBase58() === tokenMintAddress.toBase58()
      );
      const associatedTokenAccountAddress =
        await TokenProgramService.findAssociatedTokenAddress(
          userAccount.publicKey,
      ...
      if (
        !filteredTokenAccountInfos.some(
          (accountInfo) =>
            accountInfo.address.toBase58() ===
      ...
      for (let j = 0; j < filteredTokenAccountInfos.length; j++) {
        ...
        if (
          ...
          if (tokenAccountInfo.amount.gt(new BN(0))) {
            const transferTokenInstruction =
              TokenProgramInstructionService.transfer(
                userAccount.publicKey,
          ...
          const closeTokenAccountInstruction =
            TokenProgramInstructionService.closeAccount(
              userAccount.publicKey,
              tokenAccountInfo.address
            );
    ...
    if (instructions.length > 0) {
      ...
      console.info(
        `Migrated SPL-Token accounts for ${userAccount.publicKey.toBase58()}`,
        "---",
        txSign,
        "\n"
      );
  ...
  static async mint(
    ...
    let [recipientTokenAddress, createATAInstruction] =
      await this.findRecipientTokenAddress(
        connection,
        payerAccount.publicKey,
        recipientAddress,
        tokenMintAddress
    ...
    console.log(
      `Minted ${amount} token units to ${recipientTokenAddress.toBase58()}`,
      "---",
      txSign,
      "\n"
    );
  ...
  static async transfer(
    ...
    console.log(
      `Transferred ${amount} token units from ${payerTokenAddress.toBase58()} to ${recipientTokenAddress.toBase58()}`,
      "---",
      txSign,
      "\n"
    );
