...
const CREATE_ASSOCIATED_TOKEN_ACCOUNT_LAYOUT: borsh.Layout<CreateAssociatedTokenAccountRequest> =
  borsh.struct([]);
...
const CHANGE_AUTHORITY_LAYOUT: borsh.Layout<ChangeAuthorityRequest> =
  borsh.struct([
    borsh.u8("instruction"),
    borsh.u8("type"),
...
const INITIALIZE_ACCOUNT_LAYOUT: borsh.Layout<InitializeAccountRequest> =
  borsh.struct([borsh.u8("instruction")]);
...
const INITIALIZE_MINT_LAYOUT: borsh.struct<InitializeMintRequest> =
  borsh.struct([
    borsh.u8("instruction"),
    borsh.u8("decimals"),
...
const TRANSFER_LAYOUT: borsh.struct<TransferRequest> = borsh.struct([
  borsh.u8("instruction"),
  borsh.u64("amount"),
]);
...
const TOKEN_MINT_LAYOUT: borsh.struct<TokenMintData> = borsh.struct([
...
export class TokenProgramInstructionService {
  ...
  static mint(
    ...
    const keys: AccountMeta[] = [
      ...
      <AccountMeta>{
        pubkey: targetTokenAddress,
        isSigner: false,
        isWritable: true,
      },
      <AccountMeta>{
        pubkey: authorityAddress,
        isSigner: true,
        isWritable: false,
      },
    ];
  ...
  static decodeTokenAccountInfo(data: Buffer): TokenAccountInfo {
    const decodedData: TokenAccountData = BorshService.deserialize(
      TOKEN_ACCOUNT,
      data
    );
  ...
  static decodeTokenMintInfo(data: Buffer): TokenMintInfo {
    const decodedData: TokenMintData = BorshService.deserialize(
      TOKEN_MINT_LAYOUT,
      data
    );
